  ...
  <SectionWrapper className="bg-muted/50" id="planos">
    <AnimatedBlock className="text-center mb-10">
      ...
      <h2 className="text-section text-foreground mb-3">
        Planos que acompanham sua rede
      </h2>
    ...
    <div className="grid md:grid-cols-3 gap-4 md:gap-5 max-w-4xl mx-auto">
      {plans.map((plan, i) => (
        <AnimatedBlock key={plan.name} delay={i * 0.08}>
          <div
            className={`p-6 rounded-xl h-full flex flex-col transition-all duration-200 ${
              plan.highlighted
          ...
          >
            {plan.highlighted && (
              <span className="inline-block text-xs font-semibold bg-accent/20 text-accent rounded-full px-3 py-1 mb-3 self-start">
                Mais escolhido
              </span>
            )}
            ...
            <Button
              variant={plan.highlighted ? "hero-light" : "hero"}
              size="lg"
